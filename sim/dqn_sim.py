...
from utils_sim import eval_model, make_dqn_model, make_env
from io_api import load_config_from_files

torch.set_float32_matmul_precision("high")


@hydra.main(config_path="", config_name="config_sim", version_base="1.3")
def main(cfg: DictConfig):  # noqa: F821

    device = torch.device(cfg.device) if cfg.device else get_available_device()

    config_dict = load_config_from_files(
        cfg.config_files.hardware_config_path, cfg.config_files.graph_path
    )

    # Make the components
    model = make_dqn_model(config_dict, seed=cfg.env.seed, device=device)
    ...
    test_env = make_env(config_dict, seed=cfg.env.seed, device="cpu")
    ...
    collector = Collector(
        create_env_fn=make_env(config_dict, seed=cfg.env.seed, device="cpu"),
